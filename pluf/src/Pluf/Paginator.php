...
class Pluf_Paginator {
	...
	function footer() {
		if (! is_null ( $this->model )) {
			$nb_items = $this->model->getCount ( array (
					'view' => $this->model_view,
					'filter' => $this->filter () 
			) );
		} else {
		...
		$out = '<tfoot><tr><th colspan="' . count ( $this->list_display ) . '">' . $this->footer_plain () . '</th></tr></tfoot>' . "\n";
		return $out;
	}
	...
	function body() {
		...
		if (strlen ( $out ) == 0) {
			$out = '<tr><td colspan="' . count ( $this->list_display ) . '">' . $this->no_results_text . '</td></tr>' . "\n";
		}
	 ...
	 *
	 * @param
	 *        	string Text to put in the action.
	...
	function headerSortLinks($field, $title = '') {
		...
		if (strlen ( $title )) {
			if (count ( $this->sort_order ) == 2 and $this->sort_order [0] == $field and $this->sort_order [1] == 'ASC') {
	...
	function searchField() {
		...
		$url = $this->getUrl ();
		return '<tr><th class="px-table-search" colspan="' . count ( $this->list_display ) . '">' . '<form method="get" action="' . $url . '">' . '<label for="px-q">' . __ ( 'Filter the list:' ) . '</label> ' . '<input type="text" name="_px_q" id="px-q" size="30"' . ' value="' . htmlspecialchars ( $this->search_string ) . '" />' . '<input type="submit" name="submit" value="' . __ ( 'Filter' ) . '" />' . '</form></th></tr>' . "\n";
	}
