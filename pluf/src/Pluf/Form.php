...
class Pluf_Form implements Iterator, ArrayAccess {
	 ...
	 *
	 * با استفاده از این فراخوانی می‌توان تمام پارامترهای یک فرم را بررسی کرد. تفاوت
	 * اصلی این بررسی با سایر موارد این است که در اینجا تمام پارامترها به صورت همزمان
	 * در نظر گرفته خواهد شد.
	 *
	 * در صورتی که بررسی به یک خطا روبرو شده باید خطای Pluf_Form_Invalid صادر شود.
	 *
	 * @note به صورت پیش فرض تمام داده‌هایی که تهی باشد از فرهم حذف خواهد شد. در مورد
	 ...
	 */
	protected function htmlOutput($normal_row, $error_row, $row_ender, $help_text_html, $errors_on_separate_row) {
		$top_errors = (isset ( $this->errors ['__all__'] )) ? $this->errors ['__all__'] : array ();
		array_walk ( $top_errors, 'Pluf_Form_htmlspecialcharsArray' );
		$output = array ();
		$hidden_fields = array ();
		foreach ( $this->fields as $name => $field ) {
			...
			if ($field->widget->is_hidden) {
				foreach ( $bf_errors as $_e ) {
					$top_errors [] = sprintf ( __ ( '(Hidden field %1$s) %2$s' ), $name, $_e );
				}
				...
				if (strlen ( $bf->label ) > 0) {
					...
					if ($this->label_suffix) {
						if (! in_array ( mb_substr ( $label, - 1, 1 ), array (
								':',
								'?',
								'.',
								'!' 
				...
				$output [] = sprintf ( $normal_row, $errors, $label, $bf->render_w (), $help_text );
			}
		}
		if (count ( $top_errors )) {
			$errors = sprintf ( $error_row, Pluf_Form_renderErrorsAsHTML ( $top_errors ) );
		...
		if (count ( $hidden_fields )) {
			...
			if (count ( $output )) {
				...
				$last_row = substr ( $last_row, 0, - strlen ( $row_ender ) ) . $_tmp . $row_ender;
				$output [] = $last_row;
			} else {
				$output [] = $_tmp;
			}
	...
	public function render_table() {
		return $this->htmlOutput ( '<tr><th>%2$s</th><td>%1$s%3$s%4$s</td></tr>', '<tr><td colspan="2">%s</td></tr>', '</td></tr>', '<br /><span class="helptext">%s</span>', false );
	}
