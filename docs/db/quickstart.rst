...
Finally you can get the data::
    ...
    $employee = $query->getOne();

While Pluf DB is simple to use for basic queries, it also gives a huge power and
...
for "most" database operations, with Pluf, you can use Pluf DB to build ALL of your
database queries.
...
Pluf DB does not resolve conflicts between similarly named tables, but it gives you
all the options to use aliases for building a complex query.
...
Using Pluf DB in higher level ORM libraries and frameworks allows them to focus on
defining the database logic, while Pluf DB can perform the heavy-lifting of query
...
:php:class:`Connection` object. The usage changes slightly::
    ...
    $q = $c->query()->table('user')->where('id', 1)->field('name');
...
If you need to execute query that is not supported by Pluf DB, you should always
use expressions::

    $tables = $c -> expr('show tables like []', [$like_str])->get();

Pluf DB classes are mindful about your SQL vendor and it's quirks, so when you're
building sub-queries with :php:meth:`Query::dsql`, you can avoid some nasty
problems::

    $sqlite_c ->query()->table('user')->truncate();

The above code will work even though SQLite does not support truncate. That's
because Pluf DB takes care of this.
...
You can actually perform multiple operations::
    ...
    $q = $c->query()->table('employee')->where('emp_no', 1234);
    $backup_data = $q->get();
    $q->delete();
...
When you are selecting data from your database, Pluf DB will prepare and execute
...
always be the case, so be cautious. Remember that Pluf DB can support vendors
