...
class Migration
{
     ...
     * @param string $app
     *            The application key
     * @return boolean true if the installation is successful
     */
    private function installAppFromConfig($app)
    {
        ...
        $connection = Pluf::db();
        $schema = Schema::getInstance(Pluf::getConfigByPrefix('db_schema_', true));
        // Create modules
        if (array_key_exists('model', $module)) {
            $models = $module['model'];
            foreach ($models as $model) {
                $schema->createTables($connection, ModelDescription::getInstance($model));
            }
        }
        return true;
    }
