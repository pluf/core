...
class ModelRelationTest extends TestCase
{
    ...
    public function testForeignkeyRenderAndRun()
    {
        ...
        $compiler = new Graphql\Compiler('\Pluf\Test\ModelRecurse');
        $compiler->write($class_name, $filename);
        $this->assertTrue(file_exists($filename));
        include $filename;

        $rootValue = new Test\ModelRecurse($model2->id);
        ...
        $rootValue = new Test\ModelRecurse($model->id);
    ...
    {
        // create data
        $model = new Test\ManyToManyOne();
        $model->one = 'One item ';
        $this->assertTrue($model->create());

        $model2 = new Test\ManyToManyTwo();
        $model2->two = 'Two item';
        $this->assertEquals(true, $model2->create());
        ...
        $compiler = new Graphql\Compiler('\Pluf\Test\ManyToManyOne');
        $compiler->write($class_name, $filename);
        $this->assertTrue(file_exists($filename));
        include $filename;

        $rootValue = new Test\ManyToManyOne($model->id);
