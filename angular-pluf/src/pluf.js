(function() {
	...
	angular.module('pluf', [ 'pluf.paginator', 'pluf.user' ]);
	 ...
	 * مدیریت کاربران
	 * 
	 * این سرویس تنها ابزارهایی را که برای مدیریت عادی یک کاربر مورد نیاز است
	 * ارائه می‌کند. برای نمونه ورود به سیستم، خروج و یا به روز کردن تنظیم‌های
	 * کاربری. مدیریت کاربران در سطح سیستم در سرویس‌های دیگر ارائه می‌شود.
	 */
	userModule.factory('UserManager', function($rootScope, $http, $q, $window,
			User, UserProfile) {
		var manager = {
			_pool : {},
			_current : null,
			...
			_signup : function(login, firstName, lastName, email, password,
					deferred) {
				var scope = this;
				return $http({
					method : 'POST',
			...
			signup : function(login, firstName, lastName, email, password) {
				var deferred = $q.defer();
				this._signup(login, firstName, lastName, email, password,
						deferred);
				return deferred.promise;
			},
			/**
			 ...
			 * 
			 * با استفاده از این فراخوانی تنها می‌توان کاربری جاری را ویرایش
			 * کرد.
			 */
