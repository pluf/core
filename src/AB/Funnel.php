...
class Funnel
{
    ...
    public static function getFunnels()
    {
        ...
        foreach (array(
            ...
            $db->funnellogs->ensureIndex(array(
                $k => 1
            ), array(
                'background' => true
            ));
    ...
    public static function getFunnelProps($funnel, $period = 'today')
    {
        ...
        switch ($period) {
            ...
            case '7days':
                $q = array(
                    't' => array(
                        '$gte' => (int) gmdate('Ymd', time() - 604800)
                    )
                );
                break;
            case 'all':
     ...
     *            Funnel
     * @param $period string
     *            Time period 'yesterday', ('today'), '7days', 'all'
     * @param $prop string
     *            Property to filter (null)
     */
    public static function getStats($funnel, $period = 'today', $prop = null)
    {
        ...
        foreach ($db->funnellogs->find($q) as $log) {
            if (! isset($uids[$log['u'] . '##' . $log['s']])) {
                ...
                $steps[$step]['total'] = (isset($steps[$step]['total'])) ? $steps[$step]['total'] + 1 : 1;
                if ($prop) {
                    $steps[$step]['props'][$log['p'][$prop]] = (isset($steps[$step]['props'][$log['p'][$prop]])) ? $steps[$step]['props'][$log['p'][$prop]] + 1 : 1;
                }
            }
        }
            ...
            if ($steps[$i]) {
                ...
                if ($prop) {
                    ...
                    foreach ($steps[$i]['props'] as $v => $t) {
                        ...
                        $steps[$i]['sprops'][$v] = array(
                            $t,
                            $pv
                        );
                        $steps[$i]['sprops1'][$v] = array(
                            $t,
                            $tprops[$v]
                        );
                        if ($pv) {
