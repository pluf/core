 ...
 * # ***** END LICENSE BLOCK *****
 */
namespace Pluf\AB\Form;

use Pluf\AB;
use Pluf\FormInvalidException;
use Pluf\Form\Field\Integer;
use Pluf\Form\Field\Varchar;
 ...
 * process it.
 */
class MarkWinner extends \Pluf\Form
{

    protected $test = null;

    /**
     * < Store the test retrieved during validation.
     */
    public function initFields($extra = array())
    {
        $this->fields['test'] = new Varchar(array(
            'required' => true
        ));
        $this->fields['alt'] = new Integer(array(
            'required' => true,
            'min' => 0
        ));
    }
    ...
    {
        $db = AB::getDb();
        $test = $db->tests->findOne(array(
            '_id' => $this->cleaned_data['test']
        ));
        if ($test == null) {
            throw new FormInvalidException('The test has not been found.');
        }
        if (! $test['active']) {
            throw new FormInvalidException('The test is already inactive.');
        }
        if (! isset($test['alts'][$this->cleaned_data['alt']])) {
            throw new FormInvalidException('This alternative is not available.');
        }
