...
class Response
{
    ...
    {
        if (is_null($mimetype)) {
            $mimetype = Bootstrap::f('mimetype', 'text/html') . '; charset=utf-8';
        }
    ...
    function outputHeaders()
    {
        if (! defined('IN_UNIT_TESTS')) {
            ...
            foreach ($this->cookies as $cookie => $data) {
                ...
                $data = (null == $data) ? '' : $data;
                setcookie($cookie, $data, $expire, Bootstrap::f('cookie_path', '/'), Bootstrap::f('cookie_domain', null), Bootstrap::f('cookie_secure', false), Bootstrap::f('cookie_httponly', true));
            }
        } else {
            $_COOKIE = array();
            foreach ($this->cookies as $cookie => $data) {
