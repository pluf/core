...
class Pluf_L10n
{
    ...
    function loadDomain ($lang = 'en', $domain = 'pluf')
    {
        ...
        return $this->loadFile(
                $this->locale_folder . $lang . '/' . $domain . '.lang');
    }
    ...
    /**
     * Optimize a locale.
     * Convert the .lang in a .php file
    ...
    function optimizeLocale ($file)
    {
        ...
        $phpfile = Pluf::f('tmp_folder') . '/Pluf_L10n-' .
                 ...
                 '.php';
        $lines = file($file);
        $out = '<?php ' . "\n" . '/* automatically generated file from: ' . $file .
                 '  */' . "\n\n";
        $out .= '$l = array();' . "\n";
        $count = count($lines);
        for ($i = 1; $i < $count; $i ++) {
    ...
    function getAvailableLocales ($domain = '')
    {
        ...
        while ($entryname = readdir($current_dir)) {
            if (is_dir($rootdir . $entryname . '/') and
                     ($entryname != '.' and $entryname != '..') and
                     (2 == strlen($entryname))) {
    ...
    public static function getInstalledLanguages ()
    {
        ...
        foreach (Pluf::f('languages', array(
                'en'
        )) as $lang) {
