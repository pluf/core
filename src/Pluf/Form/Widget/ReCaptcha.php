...
class Pluf_Form_Widget_ReCaptcha extends Pluf_Form_Widget_Input
{
    ...
    {
        return Template::markSafe(self::getHtml($this->attrs['pubkey']));
    }

    /**
     * Gets the challenge HTML (javascript and non-javascript
     * version).
     * This is called from the browser, and the resulting
    ...
    {
        $server = ($use_ssl) ? 'https://api-secure.recaptcha.net' : 'http://api.recaptcha.net';
    ...
    public function valueFromFormData($name, $data)
    {
        ...
        $res[0] = isset($data['recaptcha_challenge_field']) ? $data['recaptcha_challenge_field'] : '';
        $res[1] = isset($data['recaptcha_response_field']) ? $data['recaptcha_response_field'] : '';
        return $res;
    }
}
