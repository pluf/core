...
class HTTP
{
    ...
    function removeTheMagic()
    {
        ...
        // if (!empty($_GET)) {
        // array_walk($_GET, '\\Pluf\\HTTP::magicStrip');
        // }
        // if (!empty($_POST)) {
     ...
     * @credit Olivier Meunier
     */
    public static function magicStrip(&$k, $key)
    {
        $k = HTTP::handleMagicQuotes($k);
    }
     ...
     * @credit Olivier Meunier
     */
    public static function handleMagicQuotes(&$value)
    {
        if (is_array($value)) {
            $result = array();
            foreach ($value as $k => $v) {
                if (is_array($v)) {
                    $result[$k] = HTTP::handleMagicQuotes($v);
                } else {
