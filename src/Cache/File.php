...
class File extends \Pluf\Cache
{
    ...
    {
        if (! Bootstrap::f('cache_file_folder', false)) {
            throw new SettingException('"cache_file_folder" setting not defined.');
        }

        $this->_debug = Bootstrap::f('debug', false);
    }
    ...
    public function _keyToFile($key)
    {
        ...
        return Bootstrap::f('cache_file_folder') . '/' . substr($md5, 0, 2) . '/' . substr($md5, 2, 2) . '/' . substr($md5, 4);
    }
}
