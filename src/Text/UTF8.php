...
class UTF8
{
    ...
    public static function detect_cyr_charset($str)
    {
        ...
        for ($i = 0; $i < $length; $i ++) {
            ...
            if ($char > 223 && $char < 256)
                $charsets['Windows-1251'] += 3;
            if ($char > 191 && $char < 224)
                $charsets['Windows-1251'] += 1;

            // ISO-8859-5
            if ($char > 207 && $char < 240)
                $charsets['ISO-8859-5'] += 3;
            if ($char > 175 && $char < 208)
                $charsets['ISO-8859-5'] += 1;
        }
        arsort($charsets);
        return key($charsets);
    }
    ...
    {
        return "\x1A" . ' !"#$%&\'()+,/;<=>?@[\]^`{|}~�' . '� ¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½�' . '�¿×÷ˇ˘˙˚˛˜˝̣̀́̃̉΄΅·βφϑϒϕϖְֱֲֳִֵֶַָֹֻּֽ־ֿ�' . '�ׁׂ׃׳״،؛؟ـًٌٍَُِّْ٪฿‌‍‎‏–—―‗‘’‚“”�' . '��†‡•…‰′″‹›⁄₧₪₫€№℘™Ωℵ←↑→↓↔↕↵' . '⇐⇑⇒⇓⇔∀∂∃∅∆∇∈∉∋∏∑−∕∗∙√∝∞∠∧∨�' . '�∪∫∴∼≅≈≠≡≤≥⊂⊃⊄⊆⊇⊕⊗⊥⋅⌐⌠⌡〈〉⑩─�' . '��┌┐└┘├┤┬┴┼═║╒╓╔╕╖╗╘╙╚╛╜╝╞╟╠' . '╡╢╣╤╥╦╧╨╩╪╫╬▀▄█▌▐░▒▓■▲▼◆◊●�' . '�★☎☛☞♠♣♥♦✁✂✃✄✆✇✈✉✌✍✎✏✐✑✒✓✔✕�' . '��✗✘✙✚✛✜✝✞✟✠✡✢✣✤✥✦✧✩✪✫✬✭✮✯✰✱' . '✲✳✴✵✶✷✸✹✺✻✼✽✾✿❀❁❂❃❄❅❆❇❈❉❊❋�' . '�❏❐❑❒❖❘❙❚❛❜❝❞❡❢❣❤❥❦❧❿➉➓➔➘➙➚�' . '��➜➝➞➟➠➡➢➣➤➥➦➧➨➩➪➫➬➭➮➯➱➲➳➴➵➶' . '➷➸➹➺➻➼➽➾' . '　、。〃〈〉《》「」『』【】〒〔〕〖〗〘〙〚〛〶' . '�' . '�ﹼﹽ' . '！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾｀｛｜｝～' . '｟｠｡｢｣､･￠￡￢￣￤￥￦￨￩￪￫￬￭￮';
    }
