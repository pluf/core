...
class AB
{
     ...
     * @param $alts array
     *            Alternatives to pick from (array(true,false))
     * @param $weights array
     *            Weights for the alternatives (null)
     * @param $desc string
     *            Optional description of the test ('')
     * @return mixed One value from $alts
     */
    public static function test($test, &$request, $alts = array(
        true,
        false
    ), $weights = null, $desc = '')
    {
        if (Bootstrap::f('pluf_ab_allow_force', false) and isset($request->GET[$test])) {
            return $alts[$request->GET[$test]];
        }
        $db = self::getDb();
        // Get or set test
        $dtest = $db->tests->findOne(array(
            '_id' => $test
        ), array(
            '_id',
            'active',
        ...
        if ($dtest == null) {
            $dtest = array(
                '_id' => $test,
                'creation_dtime' => gmdate('Y-m-d H:i:s', $request->time),
                'desc' => $desc,
                'alts' => $alts,
                'exp' => 0,
                'conv' => 0,
        ...
        $intest = $db->intest->findOne(array(
            '_id' => $test . '##' . $request->pabuid
        ), array(
            '_id',
            'alt'
        ));
        ...
        $intest = array(
            ...
            'first_dtime' => gmdate('Y-m-d H:i:s', $request->time),
            'alt' => $alt
        );
        $db->intest->update(array(
            '_id' => $test . '##' . $request->pabuid
        ), $intest, array(
            'upsert' => true
        ));
    ...
    {
        $pabuid = (isset($request->pabuid)) ? $request->pabuid : self::getUid($request);
        if ($pabuid == 'bot') {
            return;
        }
     ...
     *            Name of the funnel
     * @param $step int
     *            Step in the funnel, from 1 to n
     * @param $stepname string
     *            Readable name for the step
     * @param $request HTTP\Request
     *            Request object
     * @param $props array
     *            Array of properties associated with the funnel (array())
     */
    ...
    function process_request($request)
    {
        ...
        if (isset($request->COOKIE['pabprops'])) {
            try {
                $request->pabprops = Sign::loads($request->COOKIE['pabprops']);
            } catch (Exception $e) {}
        }
        return false;
    }
    ...
    public static function getDb()
    {
        ...
        $server = Bootstrap::f('pluf_ab_mongo_server', 'mongodb://localhost:27017');
        $options = Bootstrap::f('pluf_ab_mongo_options', array(
            'connect' => true,
            'persist' => 'pluf_ab_mongo'
        ));
        $conn = new Mongo($server, $options);
        self::$db = $conn->selectDB(Bootstrap::f('pluf_ab_mongo_db', 'pluf_ab'));
        return self::$db;
    }
    ...
    public static function getUid($request)
    {
        ...
        if (! isset($request->SERVER['HTTP_USER_AGENT']) or self::isBot($request->SERVER['HTTP_USER_AGENT'])) {
            return 'bot';
        }
    ...
    {
        $base = sprintf('%08X%08X%08X', $request->time, sprintf('%u', crc32($request->remote_addr)), rand());
        return sprintf('%s%08X', $base, sprintf('%u', crc32($base . md5(Bootstrap::f('secret_key')))));
    }
    ...
    public static function check_uid($uid)
    {
        ...
        $check = sprintf('%08X', sprintf('%u', crc32(substr($uid, 0, 24) . md5(Bootstrap::f('secret_key')))));
    ...
    public static function getTestStats($test)
    {
        ...
        for ($i = 0; $i < $n; $i ++) {
            ...
            $nconvr = ($convr !== null) ? sprintf('%01.2f%%', $convr * 100.0) : 'N/A';
            $ssize = ($convr !== null and $convr > 0) ? self::ssize($convr) : array();
            $stats[] = array(
                'alt' => $i,
                'convr' => $convr,
                'conv' => $conv,
    ...
    public static function zscore($c, $t)
    {
        ...
        $s = (self::cr($t) * (1 - self::cr($t))) / $t[0] + (self::cr($c) * (1 - self::cr($c))) / $c[0];
        return $z / sqrt($s);
    }
