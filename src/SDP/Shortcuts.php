...
function SDP_Shortcuts_GetTagByNameOr404($name) {
	...
	if ($item->count () > 1) {
		Pluf_Log::error ( sprintf ( 'more than one tag exist with the name $s in tenant $s', $name, $tenant->id ) );
		return $item [0];
	}
	throw new SDP_Exception_ObjectNotFound ( "SDP tag not found (Tag name:" . $name . ")" );
}
...
function SDP_Shortcuts_Mime2Ext($mime_type)
{
	...
	// TODO: mahdi: As you may notice the following variable must be completed with
