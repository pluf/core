...
class Geometry extends Field
{
    ...
    public function clean($value)
    {
        ...
        if ($this->max_length !== null and $value_length > $this->max_length) {
            throw new FormInvalidException(sprintf('Ensure this value has at most %1$d characters (it has %2$d).', $this->max_length, $value_length));
        }
        if ($this->min_length !== null and $value_length < $this->min_length) {
            throw new FormInvalidException(sprintf('Ensure this value has at least %1$d characters (it has %2$d).', $this->min_length, $value_length));
        }
        return $value;
    }
    ...
    {
        if ($this->max_length !== null and in_array(get_class($widget), array(
