...
class ReCaptcha extends Field
{
    ...
    public function clean($value)
    {
        ...
        // error.
        self::checkAnswer($this->privkey, $this->remoteip, $value[0], $value[1], $this->extra_params);
        return $value;
    }
    ...
    public static function httpPost($host, $path, $data, $port = 80)
    {
        ...
        if (false === ($fs = @fsockopen($host, $port, $errno, $errstr, 10))) {
            throw new FormInvalidException('Cannot connect to the reCaptcha server for validation.');
        }
    ...
    {
        if ($privkey == '') {
            throw new FormInvalidException('To use reCAPTCHA you must set your API key.');
        }
        if ($remoteip == '') {
            throw new FormInvalidException('For security reasons, you must pass the remote ip to reCAPTCHA.');
        }
        ...
        $response = self::httpPost('api-verify.recaptcha.net', '/verify', array(
            'privatekey' => $privkey,
            'remoteip' => $remoteip,
            'challenge' => $challenge,
            'response' => $response
        ...
        if (trim($answers[0]) == 'true') {
            ...
            throw new FormInvalidException($answers[1]);
        }
    }
}
