...
class User_Form_Account extends Pluf_Form
{
    ...
    private function send_validation_mail ($new_email, $secondary_mail = false)
    {
        ...
        $encrypted = trim(
                $cr->encrypt(
                        $new_email . ':' . $this->user_data->id . ':' . time() .
                                 ':' . $type), '~');
        $key = substr(md5(Pluf::f('secret_key') . $encrypted), 0, 2) . $encrypted;
        $url = Pluf::f('url_base') . Pluf_HTTP_URL_urlForView(
                'User_Views_User::changeEmail', 
                array(
                        $key
                ), array(), false);
        ...
        $email->sendMail();
        $this->user_data->setMessage(
                sprintf(
                        __(
                                'A validation email has been sent to "%s" to validate the email address change.'), 
                        Pluf_esc($new_email)));
    }
    ...
    function clean_email ()
    {
        ...
        // $user = Pluf::factory ( 'IDF_EmailAddress'
        // )->get_user_for_email_address ( $this->cleaned_data ['email'] );
        // if ($user != null and $user->id != $this->user_data->id) {
        // throw new Pluf_Form_Invalid ( sprintf ( __ ( 'The email "%s" is
        // already used.' ), $this->cleaned_data ['email'] ) );
        // }
